  ...
  useEffect(() => {
    const loadTechnicianData = async () => {
      ...
      try {
        ...
        const response = await axios.get(`https://sree-teq-project-api.onrender.com/api/auth/technician/${techid}`, {
  ...
  useEffect(() => {
    if (activeSection === 'orders') {
      const fetchOrders = async () => {
        try {
          const response = await axios.get(`https://sree-teq-project-api.onrender.com/api/payment/users`);
  ...
  const handleOrderAction = async (transactionId, actionType) => {
    try {
      if (actionType === 'complete') {
        await axios.post(`https://sree-teq-project-api.onrender.com/api/payment/complete/${transactionId}`);
        setCompletedOrders(prevState => [...prevState, transactionId]);
      } else if (actionType === 'cancel') {
        await axios.post(`https://sree-teq-project-api.onrender.com/api/payment/cancel/${transactionId}`);
